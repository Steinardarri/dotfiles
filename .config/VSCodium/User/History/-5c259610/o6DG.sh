...
if [ "$1" != "" ]; then
  ...
  find langs/is/rules/* -type d | while read -r i; do
    FLAG=$((FLAG + 1))
    RULE="$(basename "$i")"
    ...
    if [ -f "$i/aff" ]; then
      ...
      {
        echo "#$RULE"
        echo "SFX $FLAG N $LINECOUNT"
        sed "s/SFX X/SFX $FLAG/g" "$i/aff"
      } >>wiktionary.aff
    fi
  ...
  LC_ALL=is.UTF-8 sort dicts/is_IS.dic wordlist.diff -o dicts/is_IS.dic
  awk '!a[$0]++' dicts/is_IS.dic
  insertHead "wc -l < dicts/is_IS.dic" dicts/is_IS.dic

  echo "Done building dictionary, see dicts/is_IS.dic and dicts/is_IS.aff."
