...
if [ "$2" != "" ]; then
  ...
  ./makedict.py iswiktionary-"${2}"-pages-articles.xml wiktionary.dic wiktionary.aff
  ...
  do
    ...
    if [ -e "$i/print-dic-entry" ]; then
        grep -o "^{{$RULE|[^}]\+" iswiktionary-"${2}"-pages-articles.xml.texts | grep -o "|.*" | "./$i/print-dic-entry" $FLAG >> wiktionary.dic
    else
        grep -o "^{{$RULE|[^}]\+" iswiktionary-"${2}"-pages-articles.xml.texts | grep -o "|.*" | gawk -F "|" '{printf "%s%s%s\n", $1, $2, $3"/"'"$FLAG"'}' >> wiktionary.dic
    fi
  done
