...
if [ "$1" != "" && "$2" != "" ]; then
  echo "Creating a hunspell dictionary from $1..."
else
  echo "Usage: ./makehunspell.sh <language> <dump>"
  exit 0
fi
...

  {
    ...
    grep -C 3 "{{-is-}}" iswiktionary-latest-pages-articles.xml | grep -C 2 "{{-is-skammstöfun-}}" | grep "'''" | grep -o "[^']*"
    #extracting adverbs
